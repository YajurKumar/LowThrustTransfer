...
%     for j = 1:length(p.Narray)
%         Xj = X(p.cumN(j)+1:p.cumN(j+1),:); % states for segment j
%         Uj = U(p.cumN(j)+1:p.cumN(j+1),:); % controls for segment j
%         D = p.D{j}; % differentiation matrix
%         G = Gmatrix(p.tau{j},Xj,Uj,p.t0(j),p.tf(j),p); % differential matrix
%         %in_defect{j,1} = G; % defect constraints
%         in_defect{j,1} = D*Xj - G; % defect constraints
%         %in_defect{j,1} = D*Uj - G; % defect constraints
%     end
%     c = [c;reshape(cell2mat(in_defect),[],1)];
    
%     % state and control continuity constraints between segments
%     for j = 1:length(p.Narray)-1
%         in_contin{j,1} = [X(p.cumN(j+1),:) - X(p.cumN(j+1)+1,:),...
%             U(p.cumN(j+1),:) - U(p.cumN(j+1)+1,:)];
%     end
%     c = [c;reshape(cell2mat(in_contin),[],1)];

    % boundary constraints
    % boundary = eval([p.boundary,'(X,U,p)']);
    % c = [c;reshape(boundary,[],1)];
