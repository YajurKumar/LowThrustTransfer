...
switch study
    ...
    case 2
        p.Tarray = [t0,tf]; % segment boundaries
        p.Narray = 10+1; % number of nodes per segment (same size as p.Tarray)
    case 3
        p.Tarray = [t0,tf]; % segment boundaries
    ...
    case 13
        p.Tarray = linspace(t0,tf,4); % segment boundaries
        p.Narray = 5*ones(1,length(p.Tarray)-1)+1;
    case 14
        p.Tarray = linspace(t0,tf,4); % segment boundaries
        p.Narray = 10*ones(1,length(p.Tarray)-1)+1;
    case 15
        p.Tarray = linspace(t0,tf,5); % segment boundaries
        p.Narray = 2*ones(1,length(p.Tarray)-1)+1;
end
% solve the problem
[t,X,U,f,p]=MCW_solve_loop(p);
i = study;
studyno(i)  = study;
J_cost(i)   = p.J_cost;
itr(i)      = p.iterate;
func(i)     = p.funcount;
consvio(i)  = p.constvio;
%msgs(i)     = p.msg;
stepsz(i)   = p.stepsz;
sno(i)      = i;
time(i)     = p.time;
...
display = [sno;J_cost;itr;time];

fileID = fopen('record.txt','w');
%fprintf(fileID,'%s %16s %12s %400','Case','Cost','Iterations','Message');
%fprintf(fileID,'%d %16.10s %d %s',display);
fprintf(fileID,'%5s %15s %4s %5s\r\n','Case','Cost','Iterations','Time(s)');
fprintf(fileID,'%5d %15d %10d %8.4f\r\n',display);
fclose(fileID);
%MCW_plot(t,X,U,f,p);
